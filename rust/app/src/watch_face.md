...
Trick Question: What's the difference between this Rust Binding code (generated by `bindgen` for importing a C function)...
...
And this Rust Safe Wrapper function...
...
`safe_wrap` inspects the imported function name (`lv_label_set_text`), parameter types (`lv_obj_t` and `c_char`) and return type (none). 
